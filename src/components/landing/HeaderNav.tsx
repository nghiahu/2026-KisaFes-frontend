  ...
  return (
    <ul className="hidden lg:flex items-center gap-6 xl:gap-8">
      ...
      <li className="relative group">
        <Button className="flex items-center gap-1 text-sm font-semibold text-foreground hover:text-primary transition-colors duration-200 py-4">
        ...
        <div className="absolute top-full left-0 w-64 bg-white rounded-xl shadow-xl border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50 p-2">
          <ul className="flex flex-col">
            <li>
              <a href="#features" className="block px-4 py-2 text-sm text-foreground hover:bg-slate-50 hover:text-primary rounded-lg transition-colors">
                {t('landing.nav.dropdown.features.all')}
              </a>
            </li>
            <li>
              <a href="#rovo" className="block px-4 py-2 text-sm text-foreground hover:bg-slate-50 hover:text-primary rounded-lg transition-colors">
      ...
      <li className="relative group">
        ...
        <div className="absolute top-full -left-24 xl:-left-40 w-[600px] bg-white rounded-2xl shadow-xl border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50 p-6 flex gap-8">
          ...
          <div className="flex-1">
            ...
            <ul className="flex flex-col gap-1">
              {['marketing', 'engineering', 'design', 'operations', 'it'].map((item) => (
                <li key={item}>
                  <a href={`#${item}`} className="block py-1.5 text-sm text-muted-foreground hover:text-primary transition-colors">
          ...
          <div className="flex-1">
            ...
            <ul className="flex flex-col gap-1">
              <li>
                <a href="#enterprise" className="block py-1.5 text-sm text-muted-foreground hover:text-primary transition-colors">
      ...
      {/* Guide (No Dropdown) */}
      <li>
        <a href="#guide" className="text-sm font-semibold text-foreground hover:text-primary transition-colors duration-200 py-4">
          {t('landing.nav.guide')}
        </a>
      </li>
          ...
          <ul className="flex flex-col gap-0.5">
            {['all', 'software', 'finance', 'marketing', 'design', 'sales', 'operations', 'service', 'hr', 'legal', 'itOps'].map((item) => (
              <li key={item}>
                ...
                <a href={`#template-${item}`} className="block px-3 py-2 text-sm text-foreground hover:bg-slate-50 hover:text-primary rounded-lg transition-colors">
            ...
            <li>
              <a href="#kisaService" className="block px-3 py-2 text-sm font-medium text-foreground hover:bg-slate-50 hover:text-primary rounded-lg transition-colors">
      ...
      {/* Pricing (No Dropdown) */}
      <li>
        <a href="#pricing" className="text-sm font-semibold text-foreground hover:text-primary transition-colors duration-200 py-4">
