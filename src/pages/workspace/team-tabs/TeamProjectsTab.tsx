  ...
  if (isLoading) {
    return (
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="h-32 bg-background animate-pulse rounded-xl border border-border" />
        ))}
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {projects.map((project) => (
          <div 
            ...
            className="bg-card border border-border rounded-xl p-4 flex flex-col cursor-pointer shadow-sm hover:shadow-md hover:border-blue-300 transition-all group"
          >
