...
export function ProjectListRow({ task, index }: ProjectListRowProps) {
  ...
  const getTypeInfo = (type: string) => {
    const t_str = (type || '').toLowerCase();
    switch (t_str) {
      case 'epic': return { icon: <Icons.zap size={10} className="fill-current" />, bg: 'bg-violet-100', color: 'text-violet-600', label: t('list.epic_label') };
      case 'story': return { icon: <Icons.zap size={10} className="fill-current" />, bg: 'bg-emerald-100', color: 'text-emerald-600', label: t('list.story_label') };
      case 'incident':
  ...
  return (
    <tr className="border-b border-border hover:bg-background/50 transition-colors group divide-x divide-slate-200/60">
      {columns.map(col => {
        switch (col.id) {
          ...
          case 'work': return (
            <td key={col.id} style={{ width: col.width, minWidth: col.minWidth, maxWidth: col.width }} className="py-3.5 px-4 font-semibold text-foreground text-xs">
              <div className="flex items-center gap-2.5 min-w-0 w-full overflow-hidden">
                ...
                {activeEditTitleId === task.id ? (
                  <div className="flex items-center flex-1 min-w-0 gap-1" onClick={e => e.stopPropagation()}>
                    <input
                      ...
                      autoFocus
                      className="flex-1 border border-blue-400 bg-card px-2 py-1 rounded-[3px] text-xs outline-none focus:ring-1 focus:ring-blue-400 min-w-0"
                    />
                    <button onClick={(e) => { e.stopPropagation(); handleTitleSubmit(); }} className="p-1 border border-border bg-card rounded shadow-sm hover:bg-background shrink-0 flex items-center justify-center"><Icons.check size={14} className="text-foreground" /></button>
                    <button onClick={(e) => { e.stopPropagation(); setActiveEditTitleId(null); }} className="p-1 border border-border bg-card rounded shadow-sm hover:bg-background shrink-0 flex items-center justify-center"><Icons.x size={14} className="text-foreground" /></button>
                  </div>
                ) : (
                  <span
                ...
                <span className="opacity-0 group-hover:opacity-100 flex items-center gap-1.5 ml-2 transition-all shrink-0">
                  ...
                  />
                  <Icons.plus size={12} className="text-muted-foreground hover:text-muted-foreground cursor-pointer" />
                </span>
              </div>
            </td>
          );
          case 'assignee': return (
            <td key={col.id} style={{ width: col.width, minWidth: col.minWidth, maxWidth: col.width }} className="py-3.5 px-4 text-xs font-semibold">
              ...
              >
                ...
                <span className={`truncate flex-1 ${hasAssignee ? 'text-foreground font-medium' : 'text-muted-foreground font-medium'}`} title={task.assigneeName || t('list.unassigned')}>
              ...
              {isAssigneeOpen && createPortal(
                <div ref={assigneeDropdownRef} className="fixed w-[230px] bg-card border border-border shadow-2xl rounded-xl py-2 z-[9999] overflow-hidden" style={{ top: assigneeDropdownPos.top, left: assigneeDropdownPos.left }} onClick={(e) => e.stopPropagation()}>
                  <div className="px-3 pb-2 border-b border-border">
                    <div className="flex items-center gap-2 bg-background border border-border rounded-lg px-2.5 py-1.5 focus-within:border-blue-400 focus-within:bg-card transition-all">
                      ...
                      <input ref={assigneeSearchRef} type="text" placeholder={t('list.find_user')} value={assigneeSearch} onChange={(e) => setAssigneeSearch(e.target.value)} className="flex-1 text-[12px] text-foreground bg-transparent outline-none placeholder:text-muted-foreground" />
                      {assigneeSearch && <button onClick={() => setAssigneeSearch('')} className="text-muted-foreground hover:text-muted-foreground shrink-0"><Icons.x size={11} /></button>}
                    </div>
                  </div>
                  <div className="max-h-[220px] overflow-y-auto py-1">
                    ...
                    {filteredMembers.length > 0 && (
                      <>
                        {!assigneeSearch.trim() && <div className="px-3 py-1 mt-1"><span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{t('list.project_members')}</span></div>}
                    ...
                    {filteredMembers.length === 0 && assigneeSearch.trim() && <div className="px-3 py-4 text-center"><Icons.userX size={20} className="text-slate-300 mx-auto mb-1" /><p className="text-[11px] text-muted-foreground">{t('list.user_not_found')}</p></div>}
                    {projectMembers.length === 0 && <div className="px-3 py-4 text-center"><p className="text-[11px] text-muted-foreground">{t('list.no_members')}</p></div>}
                  </div>
                </div>,
                document.body
              )}
          ...
          case 'priority': return (
            <td key={col.id} style={{ width: col.width, minWidth: col.minWidth, maxWidth: col.width }} className="py-3.5 px-4 text-xs font-semibold">
              ...
              {isPriorityOpen && createPortal(
                <div ref={priorityDropdownRef} className="fixed w-[160px] bg-card border border-border shadow-2xl rounded-xl py-1.5 z-[9999] overflow-hidden" style={{ top: priorityDropdownPos.top, left: priorityDropdownPos.left }} onClick={(e) => e.stopPropagation()}>
                  {PRIORITIES.map((p) => {
            ...
            <td key={col.id} style={{ width: col.width, minWidth: col.minWidth, maxWidth: col.width }} className="py-3.5 px-4">
              <div className="relative inline-block w-full overflow-hidden">
                ...
                {isStatusOpen && createPortal(
                  <div ref={statusDropdownRef} className="fixed w-[180px] bg-card border border-border shadow-xl rounded-[4px] py-1.5 z-[9999]" style={{ top: statusDropdownPos.top, left: statusDropdownPos.left }}>
          ...
          case 'resolution': return (
            <td key={col.id} style={{ width: col.width, minWidth: col.minWidth, maxWidth: col.width }} className="py-3.5 px-4 text-xs font-semibold text-muted-foreground truncate">
              {(() => {
          ...
          case 'created': return (
            <td key={col.id} style={{ width: col.width, minWidth: col.minWidth, maxWidth: col.width }} className="py-3.5 px-4 text-xs text-muted-foreground font-medium truncate">
          ...
          case 'dueDate': return (
            <td key={col.id} style={{ width: col.width, minWidth: col.minWidth, maxWidth: col.width }} className="py-3.5 px-4 text-xs font-medium truncate group/date relative cursor-pointer hover:bg-background transition-colors" onClick={(e) => {
              const input = e.currentTarget.querySelector('input[type="date"]') as HTMLInputElement;
              if (input) { try { input.showPicker(); } catch (err) { input.focus(); } }
            }}>
              <div className="flex items-center justify-between pointer-events-none">
                {task.dueDate ? <span className="text-foreground">{new Date(task.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span> : <span className="text-muted-foreground">{t('list.none')}</span>}
                {task.dueDate && <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDueDateChange(''); }} className="opacity-0 group-hover/date:opacity-100 text-muted-foreground hover:text-rose-500 transition-opacity ml-2 z-[20] relative pointer-events-auto" title={t('list.clear_due_date')}><Icons.x size={13} /></button>}
              </div>
              <input type="date" value={task.dueDate ? task.dueDate.substring(0, 10) : ''} onChange={(e) => handleDueDateChange(e.target.value)} className="absolute w-0 h-0 opacity-0 pointer-events-none" title={t('list.edit_due_date')} />
            </td>
          );
          case 'actions': return (
            <td key={col.id} style={{ width: col.width, minWidth: col.minWidth, maxWidth: col.width }} className="py-3.5 px-4 text-center">
              ...
              }} className="opacity-0 group-hover:opacity-100 p-1 text-muted-foreground hover:text-muted-foreground hover:bg-muted rounded transition-all"><Icons.moreHorizontal size={14} /></button>
              {isActionOpen && createPortal(
                <div ref={actionDropdownRef} className="fixed w-[150px] bg-card border border-border shadow-xl rounded-md py-1 z-[9999]" style={{ top: actionDropdownPos.top, left: actionDropdownPos.left }} onClick={(e) => e.stopPropagation()}>
                  <button onClick={() => { setIsActionOpen(false); setSelectedTask(task); }} className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] font-medium text-foreground hover:bg-background transition-colors"><Icons.settings size={14} className="text-muted-foreground" /> {t('list.edit_task')}</button>
                  ...
                  <button onClick={(e) => {
                    ...
                    setDeleteConfirmText('');
                  }} className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] font-medium text-rose-600 hover:bg-rose-50 transition-colors"><Icons.trash2 size={14} className="text-rose-500" /> {t('list.delete')}</button>
                </div>,
                document.body
              )}
            </td>
